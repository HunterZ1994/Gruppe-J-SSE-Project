...
const createIndex = (userInfo) => {
    const dbAccess = userInfo.role && userInfo.role === 'vendor' ? db_connection.getArticlesOfVendor(userInfo.userId) : db_connection.getSearchedArticles();
    return new Promise((resolve, reject) => {
        Promise.all([tools.readHtmlAndAddNav(userInfo, 'index.html'),
            dbAccess]).then(results => {
